 ...
 */
extern NSString * const DNChatcaveServiceAuthRequiredNotification;

typedef void (^DNChatServiceSuccess)(NSData *data);

/**
 * The common callback block signature for remote calls that fail
 */
typedef void (^DNChatServiceFailure)(NSError *error);
 ...
 */
@interface DNChatService : NSObject
 ...
 * the root URL for all subsequent requests for a DNChatcaveService instance until
...
- (NSString *)fetchChatroomWithID:(NSString *)chatroomID
                          success:(void(^)(DNChatroom *chatroom))success
                          failure:(DNChatServiceFailure)failure;
 ...
 * an NSArray of DNChatter instances
...
- (NSString *)postMessageWithText:(NSString *)text
                       toChatroom:(NSString *)chatroomID
                          success:(void(^)(DNMessage *message))success
                          failure:(DNChatServiceFailure)failure;
...
@interface DNChatService (SubclassRequirements)
