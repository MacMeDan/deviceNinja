//
//  DNChatroomMessagesViewController.m
//  NinjaChat
//
...
@interface DNChatroomMessagesViewController () <UIAlertViewDelegate>

@property (nonatomic, strong) DNWaitView *waitView;
@property (nonatomic, strong) NSMutableSet *requests;
@property (nonatomic, strong) NSMutableArray *messages;
@property (nonatomic, strong) DNMessageTableViewCell *messageSizingCell;
@property (nonatomic, strong) NSTimer *pollingTimer;
@property (nonatomic, assign) BOOL joinedChatroom;

@end

@implementation DNChatroomMessagesViewController
...
- (void)dealloc
{
    ...
    for (NSString *requestID in self.requests) {
        [[DNChatService sharedInstance] cancelRequestWithIdentifier:requestID];
    }
}
...
- (void)setChatroom:(DNChatroom *)chatroom
{
...
- (void)viewDidLoad
{
    ...
    self.messageSizingCell = [[DNMessageTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:nil];
    
    self.waitView = [[DNWaitView alloc] initWithFrame:CGRectZero];
...
{
    DNMessage *message = self.messages[indexPath.row];

    if (message.type == DNMessageTypeLeave || message.type == DNMessageTypeJoin) {
        DNStatusMessageTableViewCell *cell = (DNStatusMessageTableViewCell *)[tableView dequeueReusableCellWithIdentifier:@"StatusCell"];
        cell.message = message;
        return cell;
    }
    else {
        DNMessageTableViewCell *cell = (DNMessageTableViewCell *)[tableView dequeueReusableCellWithIdentifier:@"MessageCell"];
...
{
    DNMessage *message = self.messages[indexPath.row];
    if (message.type == DNMessageTypeJoin || message.type == DNMessageTypeLeave) {
        return DNStatusMessageTableViewCellHeight;
    }
    else {
...
- (void)fetchMessages
{
    if (self.isViewLoaded) {
        ...
        if (self.messages.count > 0) {
            DNMessage *lastMessage = [self.messages lastObject];
            requestID = [[DNChatService sharedInstance] fetchMessagesForChatroom:self.chatroom.publicID
